...
echo
echo "  Build output directories, top 12 by size."
echo "  \"Regenerable\" assumes the toolchain still resolves. An old project"
echo "  whose dependencies no longer install cannot be rebuilt, so provenance"
echo "  is checked per directory rather than assumed from the name:"
echo "    GREEN  git-ignored inside a repo -- the repo treats it as output"
echo "    RED    tracked by git -- committed content, NOT build output"
echo "    AMBER  no repo, or untracked and not ignored -- provenance unknown"
echo

# Read-only git queries: rev-parse, ls-files, check-ignore. Nothing writes.
classify_build_dir() {
  local d="$1" repo
  command -v git >/dev/null 2>&1 || { echo "AMBER|git unavailable — cannot check provenance"; return; }
  repo=$(cd "$d" 2>/dev/null && git rev-parse --show-toplevel 2>/dev/null)
  if [ -z "$repo" ]; then
    echo "AMBER|not inside a git repo — provenance unknown"
  elif [ -n "$(git -C "$repo" ls-files -- "$d" 2>/dev/null | head -1)" ]; then
    echo "RED|TRACKED by git — this is committed content, not build output"
  elif git -C "$repo" check-ignore -q "$d" 2>/dev/null; then
    echo "GREEN|git-ignored in $(basename "$repo") — repo treats it as output"
  else
    echo "AMBER|untracked and not ignored — provenance unclear"
  fi
}

$SUDO find "$HOME" -maxdepth 6 -type d \
  \( -name target -o -name dist -o -name build -o -name out \
     -o -name .next -o -name .gradle \) \
  -prune 2>/dev/null | head -200 | \
  while IFS= read -r b; do echo "$(dir_kb "$b")|$b"; done | sort -rn -t'|' | head -12 | \
  while IFS='|' read -r k pth; do
    VERDICT=$(classify_build_dir "$pth")
    CLASS="${VERDICT%%|*}"; WHY="${VERDICT#*|}"
    printf "    %-6s %s  %s\n" "$CLASS" "$(human "$(kb "$k")")" "$pth"
    printf "           %s\n" "$WHY"
  done
...
printf "           (build output is classified per-directory in 9b, by git\n"
printf "            provenance — it is NOT included in this figure)\n"
printf "           dev caches              %s\n" "$(human "$(kb "$DEV_KB")")"
